...
def logistic_predict(weights, data):
    ...
    #####################################################################
    with_bias = np.append(data, np.ones((data.shape[0], 1)), axis=1)
    y = sigmoid(np.dot(with_bias, weights))
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
    return y
...
def evaluate(targets, y):
    ...
    y_classified[:][y_classified[:] <= 0.5] = 0.0
    y_classified[:][y_classified[:] > 0.5] = 1.0
    frac_correct = np.sum(np.equal(targets, y_classified)) / len(targets)
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
...
def logistic(weights, data, targets, hyperparameters):
    ...
    df = -np.dot(data_with_bias.T, (targets - y)) / targets.shape[0]
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
    return f, df, y
...
def logistic_pen(weights, data, targets, hyperparameters):
    ...
    f = ce + lambd * np.square(np.linalg.norm(weights[:-1])) / 2
    data_with_bias = np.append(data, np.ones((data.shape[0], 1)), axis=1)
    df = -np.dot(data_with_bias.T, (targets - y)) / targets.shape[0] + \
         lambd * weights
    #####################################################################
